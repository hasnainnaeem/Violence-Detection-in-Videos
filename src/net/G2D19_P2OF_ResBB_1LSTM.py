...
class Net(NetworkBase):
    ...
    def Build(self):
        ...
        with tf.name_scope("CNN"):
            ...
            out, updateOp1 = ResidualBlock('ResBlock', out, [512, 512, 2048], isTraining_=self._isTraining,
                                           trainingStep_=self._trainingStep, activationType_="LEAKY_RELU",
                                           isTrainable_=True)
    ...
    def _buildOpticalFlowNet(self, inputTensor_):
        ...
        with tf.name_scope("OpticalFLow"):
            ...
            out = ConvLayer('Conv2', out, filterSize_=3, numberOfFilters_=64, stride_=1, padding_='SAME',
                            isTrainable_=True)
    ...
    def _buildAudioNet(self, inputTensor_):
        '''
            The input shape = [b, u, g, w, h, c]
            after Conv, shape = [b*u*g, w', h', c']
            here, decouple the Group dimension, shape = [b*u, g * w' * h' * c']
